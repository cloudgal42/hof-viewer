...
import {type Dispatch, type SetStateAction, useCallback, useContext, useRef} from "react";
...
const CityGallery = ({city, setPage, page}: GalleryProps) => {
  ...
  return (
    <>
      {isSplitLayout ? (
        <InfiniteScroll
          next={() => setPage(a => a + 1)}
          hasMore={imageUrls.length > currImageUrls.length}
          loader={<div>Loading...</div>}
          dataLength={currImageUrls.length}
